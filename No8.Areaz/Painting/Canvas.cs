...
public class Canvas
{
    ...
    public Point Cursor { get; set; } = Point.Empty;
    ...
    {
        var clipX = _clip?.Left ?? 0;
        var clipY = _clip?.Top ?? 0;
        var clipMaxX = clipX + (_clip?.Width ?? Width);
        var clipMaxY = clipY + (_clip?.Height ?? Height);
    ...
    public void Resize(int wide, int high)
    {
        ...
        for (var y = 0; y < Math.Min(Width, wide); y++)
        {
            for (var x = 0; x < Math.Min(Height, high); x++)
                newGlyphs[y * wide + x] = this[y, x];
        }
        for (var i = 0; i < newGlyphs.Length; i++)
            newGlyphs[i] ??= new (DefaultChar, Background, Foreground);
    ...
    public void FillRectangle(Rectangle rect, Rune? chr = null, Color? foreground = null, Color? background = null)
    {
        var left = rect.Left;
    ...
    {
        var lineDraw = GetLineDrawSet(lineSet) ?? throw new ArgumentException(nameof(lineSet));
        ...
        var goingUp = endY < startY;
        var horizontal = width > Math.Abs(height);
    ...
    public void PaintBackground(
        Rectangle rect,
        Brush brush) =>
        PaintBackground(rect.Left, rect.Top, rect.Right, rect.Bottom, brush);
    public void PaintBackground(
        int           left,
        int           top,
        int           right,
        ...
        Brush brush)
    {
        if (brush is GradientBrush gradientBrush && gradientBrush.Direction == GradientDirection.Vertical)
        {
            ...
            for (var y = top; y < bottom; y++)
            {
                var percent = (y - top) / (float)height;
    ...
    public void PaintForeground(
        Rectangle rect,
        Brush     brush) =>
        PaintForeground(rect.Left, rect.Top, rect.Right, rect.Bottom, brush);
    public void PaintForeground(
        int           left,
        int           top,
        int           right,
    ...
    public void PaintBorderBackground(
        Rectangle rect,
        Brush     brush) =>
        PaintBorderBackground(rect.Left, rect.Top, rect.Right, rect.Bottom, brush);
    ...
    public void PaintBorderForeground(
        Rectangle rect,
        Brush     brush) =>
        PaintBorderForeground(rect.Left, rect.Top, rect.Right, rect.Bottom, brush);
    public void PaintBorderForeground(
        int           left,
        int           top,
        int           right,
    ...
    {
        // Rectangle Right and Bottom are outside the rectangle
        DrawRectangle(rect.Left, rect.Top, rect.Right-1, rect.Bottom-1, lineSet, foreground, background);
    }
    ...
    {
        var foreground = (foregroundBrush as SolidColorBrush)?.Color;
        var background = (backgroundBrush as SolidColorBrush)?.Color;
        ...
        Rune? c = null)
    {
        ...
        {
            var item1 = coordinates[i].Item1;
            var item2 = coordinates[i].Item2;
    ...
    internal bool PushClip(Rectangle? clip)
    {
        if (clip == null)
            return false;
