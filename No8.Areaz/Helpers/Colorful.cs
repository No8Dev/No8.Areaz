...
public readonly struct Colorful
{
    ...
    internal static double InterpolateAngle(double a0, double a1, double t)
    {
    ...
    public Colorful BlendHSV(Colorful other, double t)
    {
        ...
        return CreateHSV(InterpolateAngle(h1, h2, t), s1 + t * (s2 - s1), v1 + t * (v2 - v1));
    }
    ...
    {
        double h;
        // Oops, doubling point workaround necessary if a ~= b and both are very small (i.e. almost zero).
    ...
    public Colorful BlendHCL(Colorful other, double t)
    {
        ...
        // We know that h are both in [0..360]
        return CreateHCL(InterpolateAngle(h1, h2, t), c1 + t * (c2 - c1), l1 + t * (l2 - l1)).Clamped();
    }
    ...
    {
        double h;
        // Oops, doubling point workaround necessary if u ~= v and both are very small (i.e. almost zero).
    ...
    public Colorful BlendLUVLCH(Colorful col2, double t)
    {
        ...
        // We know that h are both in [0..360]
        return CreateLUVLCH(l1 + t * (l2 - l1), c1 + t * (c2 - c1), InterpolateAngle(h1, h2, t));
    }
