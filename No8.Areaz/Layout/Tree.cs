...
namespace No8.Areaz.Layout
{
    ...
    {
        public static TreeNode Create(INode node, ILayoutManager.ILayoutInstructions? instructions = null)
        {
        ...
        public static void Layout(TreeNode root, Size availableSize)
        {
            ...
            var sizeRequested = root.Node.SizeRequested ?? instructions.SizeRequested;
        ...
        internal static void LayoutInternal(TreeNode treeNode)
        {
            // Option to measure all the direct children from a top down perspective
            treeNode.Node.LayoutManager()?.MeasureIn(treeNode, treeNode.Children);
            ...
            treeNode.Node.LayoutManager()?.MeasureOut(treeNode, treeNode.Children);
            if (treeNode.MeasuredSize is null)
                throw new Exception($"node was not measured by container:{treeNode.Node}");
        }

        public static void Paint(Canvas canvas, TreeNode container, Rectangle? containerBounds = null)
        {
